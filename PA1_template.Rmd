...
amd$interval <- sapply(amd$interval ,function(x){y <- str_rev(substr(paste(str_rev(x) , "000", sep = ""), 1, 4))
...
```{r, echo=FALSE}
#amdAggrByInterval <- aggregate(amd$steps, by=list(Category=amd$rownumber), FUN=mean, na.rm=TRUE)
...
2. The `r mi`th interval, on average across all the days in the dataset, contains the maximum number of steps which is **`r mx`**
...
1. The total no of missing in the dataset is `r sum(is.na(amd$steps))` Data gaps are distributd per days, i.e. there are `r dim(admMissCount)[1]` days totally missed from the experiment, those days are:
...
The mean and median of the total number of steps with NA replaced taken per day are `r mnNA` and `r mdNA` correspondingly. If we compare this histrogram to the one section II we can see that low frequency area at the very beginning at the plot is now gone and the distribution is a bit more normal.
...
amdAggrByDayNA <- aggregate(amdNaReplaced$steps, by=list(Category=amdNaReplaced$date), FUN=sum, na.rm=TRUE)
names(amdAggrByDayNA) <- c("date", "total")
mnNA <- mean(amdAggrByDayNA$total)
mdNA <- median(amdAggrByDayNA$total)

amdNaReplaced$weekend <- (weekdays(amdNaReplaced$date) %in% c('Saturday','Sunday'))
stepsWE <- amdNaReplaced[amdNaReplaced$weekend==TRUE,]
stepsWD <- amdNaReplaced[amdNaReplaced$weekend==FALSE,]

amdAggrByIntervalWE <- aggregate(stepsWE$steps, by=list(Category=stepsWE$rownumber), FUN=mean)
amdAggrByIntervalWD <- aggregate(stepsWD$steps, by=list(Category=stepsWD$rownumber), FUN=mean)
names(amdAggrByIntervalWE) <- c("interval", "mean")
names(amdAggrByIntervalWD) <- c("interval", "mean")

plot(x=amdAggrByIntervalWE$interval,y=amdAggrByIntervalWD$mean,type="l",col = 'green', xlab='Interval, HH:mm', ylab="average", xaxt='n')
lines(x=amdAggrByIntervalWD$interval,y=amdAggrByIntervalWE$mean,col = 'blue')
axis(1, at=amdAggrByIntervalWE$interval, labels=c(amdAggrByInterval$time))
